...
global:
  ...
  required:
    ...
    # The chart defaults to Cloudflare's documented "always passes" test
    # tokens, so the eval install reaches the login form without you having
    # to register a real Turnstile site. PRODUCTION DEPLOYMENTS MUST OVERRIDE
    # both siteKey and secretKey with real values from dash.cloudflare.com,
    # otherwise the bot-protection layer is effectively disabled.
    turnstile:
      siteKey: "1x00000000000000000000AA"               # eval default — replace in prod
      secretKey: "1x0000000000000000000000000000000AA"  # eval default — replace in prod
