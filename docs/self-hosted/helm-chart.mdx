...
:::tip OPENAI_API_KEY across services
Every service that performs AI-piloted enrichment (`worker`, `codex-remedium`, `analysis-scheduler`, `exporter`, `plexalyzer-code`, `plexalyzer-prov`) reads the **same** `OPENAI_API_KEY` from its respective Secret. Use the same key value across all of them. The key can be a regular OpenAI API key, an Azure OpenAI endpoint key, or a DeepSeek key (set the matching `global.required.ai.openAiBaseUrlSwe` to the provider's URL).
:::
...
<Steps>
  <Step title="Create the GitOps repository">
    ...
    **Register the repo with ArgoCD.** Apply a `Secret` in the `argocd` namespace labelled `argocd.argoproj.io/secret-type: repository`. Pick the auth flavor that matches your remote.
    ...
    ```bash
    kubectl apply -f plexicus-gitops-repo.yaml
    ```
  ...
  <Step title="Create prerequisite Applications (sync-wave 0)">
    ...
    A reference manifest set is available at `argocd/prereqs/applicationset-prereqs.yaml` inside the chart artifact (extract via `helm pull --untar` from `~/plexicus-deploy`). Copy it into your GitOps repo, commit, push, then apply:
    ...
    cp plexicus/argocd/prereqs/applicationset-prereqs.yaml \
       plexicus-gitops/apps/prereqs/applicationset-prereqs.yaml

    cd plexicus-gitops
    # Edit apps/prereqs/applicationset-prereqs.yaml: replace every <TO-FILL>
    # password with a SealedSecret/ExternalSecret reference (see step 1 secrets handling).

    git add apps/prereqs/applicationset-prereqs.yaml
    ...
    kubectl apply -f apps/prereqs/applicationset-prereqs.yaml
    ```
