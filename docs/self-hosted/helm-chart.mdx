...
:::note Chart version
Examples on this page assume chart **1.2.7** or later. For older deployments, contact **[engineering@plexicus.ai](mailto:engineering@plexicus.ai)** for migration guidance.
:::
...
The procedure below was validated end-to-end on `demo.plexicus.com` (k3s + ArgoCD v3, Helm chart 1.2.7+). The reference manifest set lives in the chart repo at `argocd/prereqs/applications-prereqs.yaml` and `argocd/application.yaml` — extract via `helm pull --untar` and adapt to your environment. Key gotchas surfaced during validation:
...
- **Image-tag drift between bundled Plexicus services and the published GAR images.** The chart pins `<service>.image.tag` to the chart version (e.g. `1.2.7`), but the actual GAR images use independent build numbers (`1.0.NNN`) with a `latest` floating tag. Either override `<service>.image.tag: latest` in the umbrella's `helm.values` (acceptable for evaluation), or pin specific build tags per service for reproducible production deployments.
:::
