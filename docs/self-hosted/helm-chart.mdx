...
#### Production path — Connect SCM via OAuth \{#self-hosted_helm-chart-scm-real}

For production deployments, click **Connect SCM** instead of Go To Sandbox. This requires real OAuth client credentials populated in the `plexicus-fastapi` Secret you created in step 5 (`GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET`, plus the equivalents for GitLab / Bitbucket — see [`docs/secrets-management.md`](#self-hosted_helm-chart-day2) inside the chart artifact for the full key catalog).

Connecting a SCM is a standard OAuth 2.0 redirect flow:
...
3. Plexicus redirects you to the provider's authorize URL. Log in and click **Authorize Plexicus** (or the name of your registered OAuth app).
4. The provider redirects you back to Plexicus and the connector card displays **Connected**.

The OAuth redirect is browser-only — there is no Personal Access Token or API shortcut to skip it. Once a SCM is connected through the browser, every subsequent operation is available over the REST API for CI / scripting (`POST /create_repository_with_list`, `POST /request_repo_scan`, `GET /findings`).

After a real SCM is connected, the **Applications** page replaces the Sandbox flow:
