...
|-------------|---------|-----------|
| **GitHub App** | Scan GitHub repositories | Conditional — required if you scan GitHub |
| **GitHub OAuth** | "Login with GitHub" for end users | Optional |
| **GitLab OAuth** | Scan GitLab repositories + login | Optional |
| **Bitbucket OAuth** | Scan Bitbucket repositories + login | Optional |
| **Gitea OAuth** | Scan self-hosted Gitea repositories + login | Optional |
| **Google OAuth** | "Login with Google" for end users | Optional |
| **AI provider** (OpenAI / Azure OpenAI / DeepSeek) | AI-powered remediation and enrichment | Required for AI features; degrades gracefully without |
| **SMTP** | Email verification, invitations, password resets | **Required** |
| **Cloudflare Turnstile** | Bot protection on public signup forms | Optional — leave empty to disable |
| **Object storage** | Artifacts, scan reports, AI inputs/outputs | Required (bundled MinIO works out-of-the-box) |
| **Image registry mirror** | Mirror all custom images from your own registry — required for air-gapped or restricted networks | Optional |
| **Infrastructure prerequisites** (MongoDB, Redis, Temporal, MinIO, PostgreSQL) | Five infra services that the Plexicus chart depends on. Install each as a separate Helm release in the same namespace before installing the umbrella chart. | **Required** (the chart no longer bundles them — bundling exceeded the 1 MB Helm release Secret limit) |

For the canonical secrets-management procedure (every key per service, the `existingSecret` pattern, ready-to-paste `kubectl create secret` examples), pull the chart artifact and read the bundled `docs/secrets-management.md` — see [Where to look in the meantime](#self-hosted_configuration_index-6) below. Image-mirroring is covered in `docs/image-registry.md` bundled with the chart.
