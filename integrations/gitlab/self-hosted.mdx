...
- **Scopes**: The following scopes are required:
  - `api`: For basic API access
  - `read_repository`: For repository read access
  - `write_repository`: For repository write access
...
<Warning>
	Make sure you have selected the required scopes (`api`, `read_repository`,
	`write_repository`). The screenshot might show all scopes selected, but this
	is just for demonstration purposes. Selecting unnecessary scopes may pose
	security risks.
</Warning>
